...
import { getReportSchedules } from "@/lib/queries/report-schedules";
import { CreateReportForm } from "@/components/reports/create-report-form";
import { CreateReportScheduleForm } from "@/components/reports/create-report-schedule-form";
...
export default async function ReportsPage({
    ...
    const [{ reports }, schedules] = await Promise.all([
        getReportsPageData(membership.organizationId),
        getReportSchedules(membership.organizationId),
    ]);
        ...
        <div className="space-y-6">
            <header className="space-y-2">
                ...
                <p className="max-w-3xl text-sm leading-6 text-zinc-600">
                    Generate PDF reports manually or schedule recurring email deliveries
            ...
            <section className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
                <div className="space-y-4">
                    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <p className="mt-2 text-sm leading-6 text-zinc-600">
                            Create a one-time PDF report for a custom date range.
                        </p>
                    ...
                    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <p className="mt-2 text-sm leading-6 text-zinc-600">
                            Set up weekly or monthly PDF report delivery for a workspace user or client.
                        </p>
                ...
                <div className="space-y-4">
                        ...
                        {reports.length === 0 ? (
                            ...
                            <div className="mt-6 space-y-3">
                                {reports.map((report) => (
                                    <article
                                        key={report.id}
                                        className="flex flex-wrap items-start justify-between gap-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4"
                                    >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {report.status === "READY" && report.fileUrl ? (
                                                <a
                                                    ...
                                                    className="inline-flex h-8 items-center rounded-lg bg-zinc-950 px-3 text-xs font-medium text-white transition hover:bg-zinc-800"
                                                >
                                                    Download PDF
                                                </a>
                                            ) : null}
                    ...
                    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <div className="mt-6">
                            <ReportScheduleList orgSlug={orgSlug} schedules={schedules} />
                        </div>
                    </div>
                </div>
            </section>
